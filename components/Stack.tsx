      ...
      >
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          <motion.div variants={itemVariants} className="max-w-xl">
            <span className="font-mono text-[10px] uppercase tracking-[0.4em] text-foreground/30">[ 03_TECH_CORE ]</span>
            <h2 className="heading-wide text-4xl text-foreground mt-4">Tech Stack</h2>
            <p className="text-secondary text-xs mt-6 font-light leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-1">
          {techStack.map((tech) => (
            <motion.div 
              ...
              whileHover={{ backgroundColor: "var(--background)", borderColor: "var(--primary)" }}
              className="group relative p-8 bg-surface border border-outline flex flex-col items-center justify-center text-center transition-all duration-300 min-h-[220px]"
            >
              {/* Scanline Effect */}
              ...
              <div className="relative z-10 flex flex-col items-center">
                <span className="font-mono text-[9px] text-foreground/20 group-hover:text-primary transition-colors mb-6">[ {tech.id} ]</span>
                <span className="material-symbols-outlined text-4xl text-foreground/40 group-hover:text-foreground group-hover:scale-110 transition-all duration-500 mb-6">
                  {tech.icon}
                </span>
                <h3 className="font-mono text-[11px] uppercase tracking-widest text-foreground mb-2">{tech.name}</h3>
                <span className="font-mono text-[8px] text-foreground/30 group-hover:text-primary/60 transition-colors uppercase tracking-widest">{tech.level}</span>
              </div>
